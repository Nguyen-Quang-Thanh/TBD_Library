...
namespace TBD_library.Data.Configurations
{
    public class RegistrationConfiguration : IEntityTypeConfiguration<Registration>
    {
        public void Configure(EntityTypeBuilder<Registration> builder)
        {
            ...
            builder.HasOne(x => x.User).WithMany(x => x.Registrations).HasForeignKey(x => x.User_id).OnDelete(DeleteBehavior.ClientSetNull);
            builder.HasOne(x => x.Cinema).WithMany(x => x.Registrations).HasForeignKey(x => x.Cinema_id).OnDelete(DeleteBehavior.ClientSetNull);
        }
    }
}
