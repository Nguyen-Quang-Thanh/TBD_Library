...
namespace TBD_library.Data.Configurations
{
    public class PostInCategoryConfiguration : IEntityTypeConfiguration<PostInCategory>
    {
        public void Configure(EntityTypeBuilder<PostInCategory> builder)
        {
            ...
            builder.HasOne(x => x.Post).WithMany(x => x.PostInCategories).HasForeignKey(x => x.Post_id).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(x => x.Category).WithMany(x => x.PostInCategories).HasForeignKey(x => x.Category_id).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
